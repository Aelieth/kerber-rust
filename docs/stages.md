...
| 6 | Admin tools, plugins, propagation, remaining parity | **In tree** (`krb5-admin` ACL-enforced session, ktadd of all kvnos, kpasswd with kvno rotation, kprop dump/load, inter-realm krbtgt + transited). Kadmind version-1 framing + ONC RPC 2112 / RPCSEC_GSS; MIT `kadmin` AUTH_GSSAPI is not the oracle yet. |
...
- **Track B — Operational parity:** serving store is `RwLock` so kadmind
  mutations persist; kadmind version-1 AP-REQ framing plus `krb5-kadmind`
  ONC RPC 2112 / RPCSEC_GSS flavor 6 (MIT `kadmin` uses AUTH_GSSAPI
  300001; live `addprinc` not yet); RFC 3244 kpasswd; kprop dump/load;
  RFC 8636 SHA-256 PKINIT KDF helper exists, KDC reply key stays
  `octetstring2key`.
