...
- **Track C — Production verification:** `scripts/prod-gate.sh` (loopback
  ...
  **chaos-gate** (netem + memory + failover-under-load), and **soak-gate**
  (RSS leak check; scheduled longer run). `cargo deny`; `cargo geiger` is installed but a
...
remains. Differential-vs-MIT absent. **kprop** on 754
