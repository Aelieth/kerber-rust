...
| 7–8 | Hardening, stress, chaos, adversarial, observability, final gates | **Partial.** MIT-oracle gates exist for AS/TGS, FAST TGS `kvno`, GSS wrap, PKINIT `kinit`, SPAKE `kinit` (`pa_type` 151 / group 2), two-realm `kvno`, and SHA-2 `kinit`/`kvno`. PAC is unit-gated. Long soak and live Heimdal/AD/SSPI are environment fallbacks. |
