...
| DIR collection is not created on a read | `dir_cache_path` (`ccache_dir.rs`) requires an existing directory; only `dir_cache_path_for_store` creates it; MIT `verify_dir` (`cc_dir.c:221-240`) `mkdir`s on any resolve, so a `klist -c DIR:/path` typo creates a directory | `ccache-gate.sh` DIR list of a missing path does not create |
| Password quality fails closed like kadm5 `passwd_check` | `check_new_password` / `check_password_quality` (`krb5-kdc` `store.rs`): `""` is refused on every create and change even without a policy (`pwqual_empty.c`); under a policy a principal component, the realm, or a `[realms] dict_file` word is `KADM5_PASS_Q_DICT` (`pwqual_princ.c`, `pwqual_dict.c`); the check runs before the entry is written, so a rejected `addprinc` creates nothing; a kadm5 create with a NULL `passwd` (`kadmin addprinc -randkey`) is keyed with `random_key`, never from `""` (`insert_new_randkey`) | `c1_kadm5_create_empty_password_is_pass_q_tooshort_and_creates_nothing`; `c1_kadm5_create_null_password_is_a_random_key_not_the_empty_password`; `c1_dict_file_words_are_rejected_case_insensitively_only_with_a_policy`; `kadmin-local-gate.sh` `pwq-*` cells vs MIT `kadmin.local` |
| kpropd.acl is MIT `authorized_principal`, not a kadm5 glob | `kpropd_authorized_principal` (`krb5-admin` `kprop.rs`; `kpropd.c:1298-1348`): exact unparsed-principal prefix match ended by NUL/whitespace, optional enctype token that must name the ticket's `enc_part.enctype` (`krb5_string_to_enctype`, an unknown token skips the line), no wildcards, no leading whitespace, unopenable or unset `KRB5_KPROP_ACL` authorizes nobody; the file is re-read per connection; the check runs after the AP-REP like `kpropd.c:528` and the refusal closes the socket with the MIT log text. Before W1-C C2 the lines went through the kadm5 glob matcher, so `*@REALM` authorized every peer and an enctype restriction was ignored | `c2_kpropd_acl_exact_line_authorizes_and_wildcards_never_do`; `c2_kpropd_acl_enctype_suffix_must_name_the_ticket_enctype`; `prop-acl-gate.sh` `acl-*` cells vs MIT kpropd |
...
| Acceptor ticket kvno + etype (`rd_req_dec.c:325-347`) | `try_one_princ` → `krb5_kt_get_entry(..., tkt_kvno, tkt_etype)`; kvno 0 = any; `decrypt_try_server` iterates similar-enctype keys and uses kvno only for the error | `verify_inner` skips a key whose etype ≠ ticket etype; when `key_kvnos` is set, skip a nonzero label that is not the ticket kvno (0 is wildcard). kpasswd and `verify_init_creds` pass keytab kvnos. GSS accept leaves `key_kvnos` unset (iteration) | exact; a ticket that claims kvno N is not opened with a key labeled M≠N on the explicit path |
...
| FAST TGS strengthen-key (`fast.c:557-558, 580-593`) | Client copies `existing_key` when `strengthen_key` is NULL | `tgs.rs` `tgs_once` returns `ReplyMismatch` if the FAST TGS reply omits `strengthen_key` | **STRICTER** than MIT: a FAST TGS-REP without a strengthen-key is refused. Both KDCs always mint one (`fast_util.c:277-355`) |
...
| kadm5 create default attributes (`svr_principal.c:376-379`, `alt_prof.c:573-583`) | A create without `KADM5_ATTRIBUTES` takes `params.flags` = `[realms] default_principal_flags`, else `KRB5_KDB_DEF_FLAGS` **0** (no `REQUIRES_PRE_AUTH`) | `default_principal_flags` when the stanza is written (MIT's `krb5_flagspec_to_mask` tokens over 0 — the stanza *is* `params.flags`, `kadmin-gate.sh` `z1def` both legs `Attributes: DISALLOW_SVR`); when it is absent the kdc.conf `requires_preauth` knob (default `yes`) supplies `REQUIRES_PRE_AUTH` alone | **STRICTER**: an operator who never set `default_principal_flags` gets preauth-required principals instead of MIT's 0; set `requires_preauth = no` or an explicit `default_principal_flags` for MIT's default. Every field the request masks is applied exactly like MIT (Z1.1) |
