...
| TGS cross-TGS header PAC (`check_normal_tgs_pac`) | Client-info mismatch is 13 `HEADER_PAC` unless `is_crossrealm` and the requested server is a cross TGS and `verify_deleg_pac` succeeds (`tgs_policy.c:616-620`) | Same client-info match; the `verify_deleg_pac` slot is empty and still 13 (`ad.rs` `check_normal_tgs_pac`). Item 8 owns the real check. | **STRICTER** than MIT (an intermediate RBCD header whose PAC is for the impersonated client is refused until item 8) |
