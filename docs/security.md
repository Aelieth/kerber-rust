...
| --- | --- | --- |
| Constant-time MAC / checksum | `krb5-crypto` `mac_verify` (`derive.rs`); callers `ops.rs` decrypt/checksum, `weak.rs` RC4/DES | `crates/krb5-crypto/tests/known_answer.rs`; `decrypt_bad_mac_is_error`; `zeroize_ct.rs` `mac_verify_rejects_one_bit_flip` / `checksum_bit_flip_is_integrity` |
...
| Zeroize-on-drop — EncryptionKey | `EncryptionKey` `Drop` (`krb5-types` `lib.rs`) | `zeroize_ct.rs` `encryption_key_drop_zeroizes` |
| Zeroize-on-drop — PkinitClient | `PkinitClient` `Drop` (`krb5-protocol` `as_ex.rs`) | `zeroize_ct.rs` `pkinit_client_drop_zeroizes` |
...
| TGS authenticator checksum retry | `kdc_process_tgs_req` verifies the PA-TGS-REQ authenticator checksum over the raw request body (packet field 4) and, when that fails, retries over its own canonical re-encoding of the KDC-REQ-BODY; with no raw packet the check is skipped (`kdc_util.c:246-255`) | `issue.rs` verifies once, over the wire body only; a mismatch is 31 `PROCESS_TGS` | **STRICTER** than MIT (a client whose body is not canonical DER is refused; MIT clients emit canonical DER, so the retry is never reached by an MIT or Rust client; ledger row `kdc_util.c:112-140; :248; :691-697`) |
...
| TGS AP-REQ authenticator replay | `kdc/kdc_util.c:189-191`: `krb5_rd_req` is called with a NULL rcache — the KDC keeps no TGS replay cache, a replayed PA-TGS-REQ is processed again | `issue_tgs_body` `tgs_replay` (`ReplayCache`, 5-minute window): a replayed authenticator is 34 `REPEAT` `PROCESS_TGS` | **STRICTER** than MIT (ledger `kdc_util.c:190` and `kdc_util.c:144-191` stricter-documented; `tgs_authenticator_replay_is_repeat`) |
