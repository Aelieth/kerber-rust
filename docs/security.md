...
| Password quality fails closed like kadm5 `passwd_check` | `check_new_password` / `check_password_quality` (`krb5-kdc` `store.rs`): `""` is refused on every create and change even without a policy (`pwqual_empty.c`); under a policy a principal component, the realm, or a `[realms] dict_file` word is `KADM5_PASS_Q_DICT` (`pwqual_princ.c`, `pwqual_dict.c`); the check runs before the entry is written, so a rejected `addprinc` creates nothing | `c1_kadm5_create_empty_password_is_pass_q_tooshort_and_creates_nothing`; `c1_dict_file_words_are_rejected_case_insensitively_only_with_a_policy`; `kadmin-local-gate.sh` `pwq-*` cells vs MIT `kadmin.local` |
