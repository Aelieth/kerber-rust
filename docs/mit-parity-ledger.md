...
| kdc_util.c:233 | authenticator missing checksum | PROCESS_TGS 50 `INAPP_CKSUM` | issue.rs:620 | `TGS authenticator missing checksum` 50 | deviation | proposed: diffsend |
| kdc_util.c:248 | req-body checksum fail | PROCESS_TGS 31 `BAD_INTEGRITY` (50 only non-collision-proof type; 15 unknown; `comp_cksum` kdc_util.c:123-139) | issue.rs:617 | `TGS req-body checksum` 50 | deviation (code 50 vs 31) | `tgs_bad_checksum_is_error` (code only); proposed: diffsend body-cksum |
...
| fast_util.c:207-224 (post-E3) | verify req_checksum **then** keyed-cksum | bad bytes 41 FIND_FAST; unkeyed 12 FIND_FAST (log `Unkeyed checksum used in fast_req`); unknown type 60 FIND_FAST | preauth.rs:152-161 same order | 41/12/60 FIND_FAST (G2+G3; detail holds the MIT log text) | exact | `fast_as_rsa_md5_unkeyed_is_policy`; `fast_as_unkeyed_type_with_bad_bytes_is_modified`; `fast_as_crc32_checksum_is_generic`; `fast_as_short_mac_is_generic`; CHANGELOG G2/G3 |
