...
| dispatch.c:145-153 | empty / undecodable / unknown-tag pkt | no packet (`response == NULL` → `net-server.c:1101-1105` drop); status `MSG_TYPE` is not on the wire | `issue.rs` `handle_inner` | empty reply; TCP writes nothing | exact | diffsend `garbage-pdu`; `handle_request_empty_is_dropped` |
...
| kdc_util.c:1881-1913; net-server.c:1391-1414 | TCP length prefix > `bufsiz-4` | no status; `KRB_ERR_FIELD_TOOLONG` **61** | `listen.rs` `handle_tcp` | **61** | exact | `tcp_oversize_length_is_field_toolong` |
