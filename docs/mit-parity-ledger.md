...
Counts (after sub-plan-04 DESTROY-at-auth-layer and KDB-keytab acceptor rows):
**265** = A1 116 + A2 67 + A3 55 + A4 27.
exact 79 · stricter-documented 11 · deviation 94 ·
...
| parse.c:62-102; unparse.c:85-134; x-deltat.y:149-171,226-227; t_deltat.c:46-126; server_stubs.c:262-303,471-485,574-595,680-706; auth_acl.c:286-290 | parse/unparse quoting; `string_to_deltat` leftover unknown is EOF; CREATE/DELETE/RENAME `rec_out == NULL` authorise first; GET lookup UNK before ACL; unknown ACL op `'%c' in %s` | `KADM5_UNK_PRINC` / `KADM5_AUTH_ADD` | name.rs parse_name; deltat.rs parse; kadm5.rs dispatch_kadm5_ticket; acl.rs parse_line | `42x`→42; `3dd` invalid; authorised `user@OTHER.REALM` created; denied add is AUTH_ADD; missing GET/MODIFY/SETKEY/PURGE/EXTRACT/SET_STRING is UNK | exact | `create_foreign_realm_authorises_first`; `create_foreign_realm_then_getprinc`; `getprinc_foreign_realm_is_unk_princ`; `stub_setup_unk_before_acl_on_modify_setkey_purge_extract_setstr`; `scripts/kadmin-gate.sh` addprinc OTHER.REALM / 42x / aZ / 3dd both legs |
