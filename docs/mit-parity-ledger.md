...
Counts (after sub-plan-05 rows and the sub-plan-07 re-check `SYSTEM_ERR` row):
**271** = A1 116 + A2 67 + A3 55 + A4 33.
exact 82 · stricter-documented 12 · deviation 96 ·
...
| svr_policy.c:83-109,292-322 | create: DUP before the non-printable-name check, unset lifetimes zeroed, `BAD_MIN_PASS_LIFE` only with both mask bits and `pw_max_life != 0`; modify validates merged `pw_max_life` / length / class / history | `BAD_POLICY` / `BAD_MASK` / `BAD_LENGTH` / `BAD_CLASS` / `BAD_HISTORY` / `BAD_MIN_PASS_LIFE` | kadm5.rs CREATE_POLICY; kadm5.rs policy_floor_err; kadm5.rs MODIFY_POLICY | same codes on the masked record; an unmasked wire `pw_max_life` is ignored | exact | `modify_policy_below_floor_is_bad_length`; `modify_policy_below_floor_is_bad_class`; `modify_policy_below_floor_is_bad_history`; `modify_policy_min_life_over_merged_max_is_bad_min_pass_life`; `create_policy_dup_before_floors`; `create_policy_dup_checked_before_name`; `create_policy_min_life_without_max_is_ok`; `scripts/kadmin-gate.sh:1292-1301` / `:2021-2027` unmasked `pw_max_life` create → `getpol` identical on both legs |
...
| svr_principal.c:614-625,1336 | `modify_principal` with `KADM5_POLICY` and no explicit `pw_expiration` sets it to `last_pwd_change + pw_max_life` (0 without a max life); chpass sets `now + pw_max_life` | `Password expiration date` | store.rs apply_pw_max_life_in; store.rs chpass | same bases | exact | `scripts/kadmin-gate.sh:418-421` / `:1482-1501` backdate `user` to 1000000000 (Rust `krb5-kdb setlastpwd`, MIT `kdb5_util dump`/tl-data 1/`load`); `:1950-1955` `Last password change` and `Password expiration date` identical on both legs after `modprinc -policy life`; `:1226-1228` / `:1959-1961` admin cpw moves it off 2001 |
...
| ss_wrapper.c:66-76; kadmin.c:89-99 | `kadmin.local -q` exits 0 after a failed verb (`error()` sets `exit_status` only in script mode) | rc 0 | krb5-kadmin-local.rs main | rc 1 after a failed verb | stricter-documented | `scripts/kadmin-local-gate.sh:119` (`unknown flag` rc≠0); `docs/security.md` kadmin.local paragraph |
