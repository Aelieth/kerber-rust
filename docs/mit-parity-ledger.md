...
| kdc_util.c:784-786 | server `DISALLOW_ALL_TIX` (AS) | `SERVICE LOCKED OUT` **7** | `issue.rs:1679-1680` | `SERVICE LOCKED OUT` **7** | exact | proposed: diffsend `as-service-locked` (no e_text unit); TGS is `SERVER LOCKED OUT` `issue_acl_ap.rs:1234` |
...
| ORDER kdc_util.c:769-797 `validate_as_request` | MIT POSTDATE → CLIENT LOCKED → SERVICE LOCKED → SERVICE NOT ALLOWED; SERVICE EXPIRED (`:753`) before REQUIRED PWCHANGE (`:762`) | same statuses | issue.rs:1679-1688 flags after times; lockout at :293 | POSTDATE/locked/svr interleave differs; REQUIRED PWCHANGE merged with pw_expiration | deviation | proposed: diffsend as-postdate-and-locked; as-expired-service-and-needchange |
