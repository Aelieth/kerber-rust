...
exact 54 · stricter-documented 11 · deviation 96 ·
...
| do_tgs_req.c:536 `db_get_svc_princ` | server lookup fail | LOOKING_UP_SERVER 7 (remap :575) | issue.rs:697 | `unknown server` 7 | deviation | proposed: diffsend missing host; proposed: flags-gate.sh/kdc-gate.sh cell |
...
| do_tgs_req.c:283 | 2nd ticket server lookup/key | 2ND_TKT_SERVER (7 typical) | ad.rs:337 / u2u ad.rs:410 | `evidence server`/`evidence key` 7; U2U uses local krbtgt | deviation | proposed: diffsend; proposed: s4u-mit-gate.sh extra ticket |
...
| tgs_policy.c:72 | `VALIDATE` without TKT `INVALID` | VALIDATE VALID TICKET 13 | issue.rs:1046 | `VALIDATE VALID TICKET` 13 | exact | `tgs_renew_and_validate_together_is_badoption` (Rust-only combo); proposed: diffsend VALIDATE of already-valid TGT |
...
| tgs_policy.c:100 | TKT `INVALID` and not `VALIDATE` | TICKET NOT VALID 33 | issue.rs:1058 | `INVALID` 33 | deviation | `as_postdated_is_invalid_until_validate` (code); proposed: diffsend e_text |
...
| tgs_policy.c:241 | `RENEW` and now > `renew_till` | TKT_EXPIRED 32 | issue.rs:1071 | `renew_till` 32 | deviation | `tgs_renew_rejects_renew_till_not_after_now` (code); proposed: diffsend e_text |
| tgs_policy.c:636 | `NON_TGT_OPTION` and `tkt.server != req.server` | SERVER DIDN'T MATCH TICKET FOR RENEW/FORWARD/ETC 26 | issue.rs:692 RENEW/VALIDATE only | `RENEW/VALIDATE server mismatch` 13; FORWARD/PROXY unchecked | deviation | `tgs_renew_wrong_sname_is_badoption` (13); proposed: diffsend FORWARD mismatch |
...
| tgs_policy.c:436 | evidence not forwardable | EVIDENCE_TKT_NOT_FORWARDABLE 13 | ad.rs:347 | `S4U2Proxy evidence ticket is not forwardable` 13 | deviation | `s4u2proxy_rejects_non_forwardable_evidence` (code); proposed: s4u-mit-gate.sh |
...
| tgs_policy.c:480 | same-realm evidence server ≠ header client | EVIDENCE_TICKET_MISMATCH 26 | ad.rs:331 | `evidence sname must match TGT client` 13 | deviation | proposed: diffsend; proposed: s4u-mit-gate.sh |
...
| tgs_policy.c:569 | KDB deny RBCD/classic | NOT_ALLOWED_TO_DELEGATE 13 (same string as :449 but 13 not 12) | ad.rs:363,368 | `RBCD not allowed` / `constrained delegation not allowed` 13 | deviation | `s4u2proxy_classic_denied_without_allowed_to`; proposed: s4u-mit-gate.sh |
...
| tgs_policy.c:763 `check_anon` kdc_util.c:703 | `restrict_anon` + anon client + non-local-TGS server | ANONYMOUS NOT ALLOWED 12 | issue.rs:777 anon crealm only skips transited | no restrict_anon deny | absent | proposed: diffsend WELLKNOWN/ANONYMOUS TGS to host; proposed: kdc-gate.sh; F4 |
...
| kdc_util.c:733-738 | `now > client.expiration` | `CLIENT EXPIRED` **1** (vague→**60**) | `issue.rs:1699-1702` | `CLIENT EXPIRED` **1** | exact | `issue_acl_ap.rs::as_rejects_expired_principal_before_expired_password`; `scripts/expire-gate.sh` |
| kdc_util.c:743-749 | `now > pw_expiration` && !PWCHANGE_SERVICE | `CLIENT KEY EXPIRED` **23** | `issue.rs:1703-1707` `pw_lapsed` | `CLIENT KEY EXPIRED` **23** | exact | `issue_acl_ap.rs::as_rejects_expired_password_unless_pwchange_service`; `scripts/expire-gate.sh` |
...
| kdc_util.c:753-755 | `now > server.expiration` | `SERVICE EXPIRED` **2** | `issue.rs:1709-1711` | `SERVICE EXPIRED` **2** | exact | `issue_acl_ap.rs::tgs_rejects_expired_server` (TGS); proposed: diffsend `as-service-expired` |
...
| kdc_util.c:784-786 | server `DISALLOW_ALL_TIX` (AS) | `SERVICE LOCKED OUT` **7** | `issue.rs:1724-1726` | `SERVICE LOCKED OUT` **7** | exact | proposed: diffsend `as-service-locked` (no e_text unit); TGS is `SERVER LOCKED OUT` `issue_acl_ap.rs:1234` |
...
| policy.c:103-136 | kdcpolicy module endtime + deny (`check_kdcpolicy_as`) | plugin status + ret | `plugins.rs:349-375` stub | no MIT plugin load | deferred | none (promotion: MIT kdcpolicy module times-rewrite vs DefaultPolicy) |
...
| kdc_util.c:1768-1801 + :822-824; do_as_req.c:316-320 | RFC 6806 FAST nego: if PA 149, enc_padata 149 cksum + empty 136; ticket flag ENC_PA_REP | success silent; fail `KDC_RETURN_ENC_PADATA` | issue.rs:1173-1189 `encrypted_pa_data: None`; no ENC_PA_REP bit | MIT kinit writes no `fast_avail`/`pa_type` (client fast.c:626-671) | absent | proposed: `scripts/mit-fast-nego-gate.sh` (kinit then `klist -C` `fast_avail`); proposed: diffsend `enc_padata` |
...
| fast_util.c:277-355 + :427-440 | FAST reply always strengthen_key; CF2 replykey (`kdc_fast_response_handle_padata` / `kdc_fast_handle_reply_key`; no `kdc_fast_strengthen_reply_key` symbol). MIT client copies existing_key when strengthen_key is NULL | silent | AS issue.rs:492-503 exact; TGS issue.rs:943-951 `strengthen=None`, enc with subkey/session | same | deviation (parity, not interop-breaking) | `fast_as_exchange_strengthen_and_finished` (AS only); proposed TGS strengthen pin in mit-fast-kdc-gate |
...
| fast_util.c:53-59 | armor `krb5_rd_req` failure (bad key / unknown server / expired) | `FIND_FAST` + inner code (log `%s while handling ap-request armor`) | preauth.rs:178-205 | `FIND_FAST` 35 / 31 / 33 / 32 (G3; NYV is 33; H1 corrupt enc is 31) | exact | `explicit_as_armor_invalid_tgt_is_tkt_nyv`; `fast_as_corrupt_enc_fast_req_is_bad_integrity_find_fast`; proposed: diffsend |
...
| auth_acl.c:330-338,472-545,638-648 | kadm5.acl target + restrictions; rename = delete src AND add dest without restrictions; unparseable target / unknown restriction is `acl_init` fail | AUTH_ADD / AUTH_INSUFFICIENT | `acl.rs` parse/check/check_rename | target-scoped; `-clearpolicy` imposed | exact | `acl_target_pattern_scopes_add_and_delete`; `scripts/kadmin-gate.sh` scoped addprinc/renprinc/restricted create |
...
| enc_rc4.c:17-35 | RC4 usage `3→8, 9→9, 23→13` | n/a | `weak.rs`/`ops.rs` `arcfour_translate_usage` | 9 is 9 | deviation | no live rc4 session on this image (MIT `kinit` rc4 is `ETYPE_NOSUPP`; Rust bootstrap has no rc4 keys); unit `arcfour_usage_9_is_9` |
| schpw.c:47-82,384-397; net-server.c:1103 | kpasswd pre-AP-REQ length/version; `goto bailout`; dispatch `respond(..., NULL)` | no datagram; log `Message stream modified` / `Requested protocol version not supported` + ` - while dispatching (udp)` | `listen.rs` `handle_kpasswd_from` | `Err` those com_err texts; UDP log suffix | exact | `kpasswd_unknown_version_is_bad_version`; `kpasswd_inconsistent_length_is_malformed`; `scripts/kpasswd-gate.sh` raw both legs |
| ap_req.rs:349; safe_priv.rs:120; gss `lib.rs:505,563` | acceptor / KRB-SAFE / GSS verify uses session checksum, ignores declared cksumtype | MIT `krb5_c_verify_checksum` matches `ctp` | `verify_checksum` | declared type unused | deviation | W1-B/C; proposed: unit per site |
| kadm5_code `AclDenied` | add/delete ACL denial | MIT `AUTH_ADD` / `AUTH_DELETE` | `kadm5.rs:1842` | `AclDenied` → `AUTH_GET` (create path now `AUTH_ADD`) | deviation | W1-C; `acl_target_pattern_scopes_add_and_delete` (create is AUTH_ADD) |
