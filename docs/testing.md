...
- `scripts/differential-gate.sh` — one dump, two live KDCs (Rust `:8888`,
  MIT 1.22.2 `krb5kdc` `:88`). `crates/krb5-tools/src/bin/diffsend.rs` encodes each
  ...
  Compare lives behind `krb5-protocol` feature `diff` (`crates/krb5-tools/src/bin/diffsend.rs`
...
- `scripts/soak-gate.sh` — C2c: sustained moderate load (~120 s in CI,
  ...
  Clock skew too great (proves the preload); default `kdc_timesync`
  recovers on both (rc=0, `klist` `user@KERBER.TEST`)
  (`get_in_tkt.c:260-270`). `gss-mit-client` → Rust acceptor
  replay is 34.
