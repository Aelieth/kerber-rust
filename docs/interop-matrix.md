...
| `capaths-transit-gate.sh` | MIT `kvno` A.TEST→B.TEST→C.TEST vs three MIT KDCs then three Rust KDCs | EncTicketPart transited `tr-type=1` contents `B.TEST` and `T` match MIT KDC; deny: `KDC policy rejects request` | mit-extra |
