...
| `mit-fast-kdc-gate.sh` | MIT `kinit -T` + `kvno` vs Rust KDC; forged-realm armor vs MIT + Rust | TRACE `Upgrading to FAST due to presence of PA_FX_FAST`; ≥2 `fast::KrbFastResponse` (AS + TGS); forged `kinit -T` is 35 `NOT_US` / `The ticket isn't for us` both sides | mit-extra |
...
| `s4u-mit-gate.sh` | MIT `kvno -U` / `-U -P` vs Rust KDC; mismatch cell vs MIT + Rust | `klist` `for client user@KERBER.TEST`; user-TGT/host S4U is 36 on both KDCs; `kvno -U nosuch` not found; `kvno -U locked` revoked; non-forwardable → `BADOPTION` | mit-extra |
