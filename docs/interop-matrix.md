...
| `client-differential-gate.sh` | MIT + Rust `kinit`/`kvno` vs the MIT KDC through `kdc-req-proxy.py` | 11 seeded flows (plain/preauth/FAST/SPAKE/PKINIT/`-R`/`-k`/kvno/`-U`/`--u2u`/`-n`); CORE request fields match; MIT `klist -C -f -e -a` over both FILE caches; seven CLI error paths non-zero both sides; +3d `LD_PRELOAD` skew: MIT `kdc_timesync=0` is Clock skew, default timesync recovers (rc=0), Rust rejects AS-REP authtime; `gss-mit-client` → Rust acceptor replay 34 | mit-extra |
