...
| MS-PAC / MS-RPCE | PAC | NDR32 `KERB_VALIDATION_INFO` in field-encounter referent order. Golden `tests/traces/pac-kbruser.ndr`. Buffers 1,10,12,16,17,18,19,6,7. Signatures 6, 7, 16 (`PAC_TICKET_CHECKSUM`), 19 (`PAC_FULL_CHECKSUM`). Store SID/RID. TGS copies a verified presented TGT PAC (LOGON_INFO) and re-signs; type-16 over original EncTicketPart bytes (PAC ad-data `0x00`), not a rasn re-encode. MIT TGTs without a PAC still synthesize. Samba L1: `scripts/samba-pac-verify-gate.sh`. |
