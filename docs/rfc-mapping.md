...
| [6806](https://www.rfc-editor.org/rfc/rfc6806) | Kerberos Principal Name Canonicalization and Cross-Realm Referrals | `NT-ENTERPRISE` (10) is a single `user@suffix` component. A suffix **equal** to the realm (RFC 4120 §6.1 exact octets) maps to the stored `user`; any other suffix is **not** a local alias (`C_PRINCIPAL_UNKNOWN`; no enterprise AS referral). AS canonicalize returns the stored principal (`user`), not the enterprise string. Client: `kinit -E`. Combined `-E --pkinit`: RFC 8636 `partyUInfo` is the AS-REQ cname; SAN binding uses the issued cname. No live gate covers that combo. Gate: `scripts/rust-kinit-enterprise-gate.sh`. |
...
| [4121](https://www.rfc-editor.org/rfc/rfc4121) | GSS-API Kerberos V5 Mechanism | `krb5-gss` wrap/unwrap/MIC with 16-byte tokens, sequence numbers, 0x8003 channel bindings, send-side RRC=0 (MIT), `GSS_C_DELEG_FLAG` KRB-CRED (ticket session key, usage 14), and AES/RFC8009 `wrap_iov`/`unwrap_iov` (HEADER=GSS+confounder, empty PADDING, TRAILER=E(header)+HMAC; `SIGN_ONLY` in the Ki HMAC). Export/import is a private Rust token. MIT `libgssapi_krb5` interop is out-of-process (`scripts/gss-gate.sh`). |
| [4178](https://www.rfc-editor.org/rfc/rfc4178) | SPNEGO | `NegTokenInit`/`NegTokenResp` with krb5 `mechListMIC`. MIT initiator `gss_mech_spnego` completes against `krb5-gss-accept`. |
| [4556](https://www.rfc-editor.org/rfc/rfc4556) | PKINIT | Oakley MODP 2048/4096 (RFC 3526) plus ECDH P-256. CMS SignedData is **mandatory** against a provisioned CA (no unverified fallback). Path validation is single-level: leaf `notBefore`/`notAfter`, trust-anchor `basicConstraints` CA=true, `keyCertSign`, anchor validity, issuer/subject DN. Realm compares are exact (RFC 4120 §6.1). The client accepts a KDC CMS only when the signer has EKU `id-pkinit-KPKdc` and SAN `krbtgt/REALM@REALM` (RFC 4556 §3.2.4) **before** ECDH/KDF; `eContentType` must be `id-pkinit-DHKeyData`. The KDC requires EKU `id-pkinit-KPClientAuth` and SAN↔**issued** cname (RFC 4556 §3.2.2; NT-ENTERPRISE is compared after canonicalization). AuthPack `paChecksum` is SHA-1 of `KDC-REQ-BODY` (the FAST **inner** body when PA-FX-FAST is present). CMS `signedAttrs` are required (RFC 5652 §5.3; eContentType ≠ id-data). `PKAuthenticator` `ctime`/`cusec` are checked against the KDC skew window and the PA replay cache. CMS `eContentType` is bound to the signed `content-type` attribute (RFC 5652 §5.3); the KDC requires client `id-pkinit-authData`. CA is **opt-in** (`--export-pkinit` / `KRB5_ENABLE_PKINIT=1`). Reply key is RFC 4556 `octetstring2key` unless RFC 8636 KDF is selected. `clientPublicValue` is SPKI; KDC CMS uses `signedAttrs` + issuerAndSerialNumber. `scripts/pkinit-gate.sh` **fails** if MIT `pkinit.so` is missing or MIT `kinit` PKINIT fails (MIT 1.22.2 `kinit` with FILE identity **passes**; SAN≠cname is `pkinit client san`). Reverse: `scripts/rust-kinit-pkinit-gate.sh` (`kinit --pkinit FILE:` vs MIT; client-cert KDC identity is `pkinit kdc eku`; MIT not listening is red). |
| [6113](https://www.rfc-editor.org/rfc/rfc6113) | FAST | `PA-FX-FAST` rasn CHOICE `{ armored-data [0] }`. AS armor is explicit AP-REQ; TGS FAST uses implicit PA-TGS-REQ armor (usage 7) and checksums the AP-REQ bytes (MIT 1.22.2). Client: `kinit --fast --armor-ccache` (`scripts/rust-kinit-fast-gate.sh`; MIT 1.22.2 TRACE is `Decrypted AP-REQ`, not `FX-FAST`). KDC: `kdc-gate.sh` MIT `kvno`. PRF+ counter is prepended per RFC 6113 §5.1. |
| [SPAKE](https://datatracker.ietf.org/doc/draft-ietf-kitten-krb-spake-preauth/) | SPAKE preauth | MIT 1.22.2 SPAKE2-P256: group id 2, IANA compressed M/N, `wbytes` from PRF+ of the long-term key with seed `SPAKEsecret` plus group id, K'[n] via CF2, factor usage 65, `KDC_ERR_MORE_PREAUTH_DATA_REQUIRED` (91). `scripts/spake-gate.sh` requires MIT `kinit` TRACE `SPAKE` and `klist user@KERBER.TEST`. Reverse: `scripts/rust-kinit-spake-gate.sh` (`kinit --spake` vs MIT; TRACE `SPAKE response received` or `SPAKE derived K'`). |
