...
| [4556](https://www.rfc-editor.org/rfc/rfc4556) | PKINIT | ECDH P-256 reply-key on AS. `signedAuthPack` / `dhSignedData` are CMS SignedData with a CA-issued P-256 leaf and ECDSA-SHA256. `PrincipalStore::pkinit_anchor_pem` is a MIT `pkinit_anchors = FILE:` trust anchor. The current MIT 1.22.2 harness image has no `pkinit.so` (built without OpenSSL); `scripts/pkinit-gate.sh` still asserts the CA PEM. Raw inner DER is still accepted. |
| [6113](https://www.rfc-editor.org/rfc/rfc6113) | FAST | `PA-FX-FAST` CHOICE `{ armored-data [0] }`, armor AP-REQ, PA-FX-COOKIE, KrbFastReq/Rep, strengthen-key (KRB-FX-CF2) on the AS path. TGS FAST is not sent (MIT FIND_FAST vs PA-TGS-REQ usage 7). |
