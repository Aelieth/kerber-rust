...
| [6806](https://www.rfc-editor.org/rfc/rfc6806) | Kerberos Principal Name Canonicalization and Cross-Realm Referrals | `NT-ENTERPRISE` (10) is a single `user@suffix` component. AS canonicalize returns the stored principal (`user`), not the enterprise string. Client: `kinit -E`. Gate: `scripts/rust-kinit-enterprise-gate.sh`. |
...
| [4556](https://www.rfc-editor.org/rfc/rfc4556) | PKINIT | Oakley MODP 2048/4096 (RFC 3526) plus ECDH P-256. CMS SignedData is **mandatory** against a provisioned CA (no unverified fallback). CA is **opt-in** (`--export-pkinit` / `KRB5_ENABLE_PKINIT=1`). Reply key is RFC 4556 `octetstring2key` unless RFC 8636 KDF is selected. `clientPublicValue` is SPKI; KDC CMS uses `signedAttrs` + issuerAndSerialNumber. Client certs carry `id-pkinit-san`. `scripts/pkinit-gate.sh` **fails** if MIT `pkinit.so` is missing or MIT `kinit` PKINIT fails (MIT 1.22.2 `kinit` with FILE identity **passes**). Reverse: `scripts/rust-kinit-pkinit-gate.sh` (`kinit --pkinit FILE:` vs MIT). |
