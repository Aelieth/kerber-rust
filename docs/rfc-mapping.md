...
| [4121](https://www.rfc-editor.org/rfc/rfc4121) | GSS-API Kerberos V5 Mechanism | `krb5-gss` wrap/unwrap/MIC with 16-byte tokens, sequence numbers, 0x8003 channel bindings, send-side RRC=16, and `GSS_C_DELEG_FLAG` KRB-CRED (ticket session key, usage 14). MIT `libgssapi_krb5` interop is out-of-process (`scripts/gss-gate.sh`). |
