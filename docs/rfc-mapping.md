...
| [6806](https://www.rfc-editor.org/rfc/rfc6806) | Kerberos Principal Name Canonicalization and Cross-Realm Referrals | `NT-ENTERPRISE` (10) is a single `user@suffix` component. A suffix matching the realm (ASCII case-insensitive) maps to the stored `user`; any other suffix is **not** a local alias (`C_PRINCIPAL_UNKNOWN`; no enterprise AS referral). AS canonicalize returns the stored principal (`user`), not the enterprise string. Client: `kinit -E`. Combined `-E --pkinit`: RFC 8636 `partyUInfo` is the AS-REQ cname; SAN binding uses the enterprise local-part. No live gate covers that combo. Gate: `scripts/rust-kinit-enterprise-gate.sh`. |
