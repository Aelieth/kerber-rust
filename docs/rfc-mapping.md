...
| MS-SFU | S4U2Self / S4U2Proxy | TGS `issue_tgs`: PA-FOR-USER (HMAC-MD5-ARCFOUR cksumtype -138 on AES session keys, MIT 1.22.2); S4U2Proxy requires a **forwardable** evidence ticket (`BADOPTION` otherwise) and decodes PA-PAC-OPTIONS (167). Gate: `scripts/s4u-mit-gate.sh` MIT `kvno -U` / `-U -P`. |
...
| [4120] transited / referrals | Cross-realm | `krbtgt/FOREIGN` keys with an explicit shared key (`KRB5_TEST_INTERREALM_KEY`); optional `KRB5_TEST_INTERREALM_KEY_ACCEPT` when Windows TDO inbound/outbound AES salts differ. Referral TGTs omit PAC. `scripts/cross-realm-gate.sh` MIT `kvno host/svc.other.test@OTHER.TEST`. Live `AD.KERBER.TEST`↔`KERBER.TEST` host tickets: `scripts/ad-mit-trust-gate.sh`. |
