...
/// Expand `input` to exactly `out_len` octets.
///
/// # Errors
///
...
mod tests {
    ...
    fn rfc3961_appendix_a1() {
        assert_eq!(nfold(b"012345", 8).unwrap(), hex("be072631276b1955"));
        assert_eq!(nfold(b"password", 7).unwrap(), hex("78a07b6caf85fa"));
        assert_eq!(
            nfold(b"Rough Consensus, and Running Code", 8).unwrap(),
            hex("bb6ed30870b7f0e0")
        );
        assert_eq!(
            nfold(b"password", 21).unwrap(),
            hex("59e4a8ca7c0385c3c37b3f6d2000247cb6e6bd5b3e")
        );
        assert_eq!(
            nfold(b"MASSACHVSETTS INSTITVTE OF TECHNOLOGY", 24).unwrap(),
        ...
        assert_eq!(
            nfold(b"ba", 21).unwrap(),
            hex("fb25d531ae8974499f52fd92ea9857c4ba24cf297e")
        );
        assert_eq!(nfold(b"kerberos", 8).unwrap(), hex("6b65726265726f73"));
        assert_eq!(
            nfold(b"kerberos", 16).unwrap(),
            hex("6b65726265726f737b9b5b2b93132b93")
        );
        assert_eq!(
            nfold(b"kerberos", 21).unwrap(),
            hex("8372c236344e5f1550cd0747e15d62ca7a5a3bcea4")
        );
        assert_eq!(
            nfold(b"kerberos", 32).unwrap(),
            hex(
