...
fn kcm_status(code: i32) -> io::Error {
    ...
    let msg = match code {
        KRB5_FCC_NOFILE => "No credentials cache found".to_owned(),
        KRB5_CC_NOSUPP | KRB5_FCC_INTERNAL => format!("KCM operation unsupported ({code})"),
        KRB5_CC_IO => format!("KCM I/O ({code})"),
        _ => format!("KCM error {code}"),
    };
...
fn unsupported(err: &io::Error) -> bool {
    let s = err.to_string();
    s.contains(&KRB5_CC_NOSUPP.to_string())
        || s.contains(&KRB5_FCC_INTERNAL.to_string())
        || s.contains(&KRB5_CC_IO.to_string())
}
...
fn try_replace(io: &mut KcmIo, name: &str, cc: &FileCcache) -> io::Result<()> {
    let creds: Vec<&CcacheCred> = cc.creds.iter().filter(|c| !c.is_removed()).collect();
    let mut args = zname(name);
    args.extend_from_slice(&0u32.to_be_bytes());
    args.extend_from_slice(&marshal_primary(&cc.primary.0, &cc.primary.1));
    args.extend_from_slice(&(u32::try_from(creds.len()).unwrap_or(0)).to_be_bytes());
    for c in creds {
        let raw = marshal_one_cred(c);
        args.extend_from_slice(&(u32::try_from(raw.len()).unwrap_or(0)).to_be_bytes());
        args.extend_from_slice(&raw);
    }
    io.call(OP_REPLACE, &args).map(|_| ())
}
...
/// Store a FILE v4 cache into `KCM:residual`.
///
/// # Errors
///
...
pub fn kcm_store(residual: &str, cc: &FileCcache) -> io::Result<()> {
    ...
    match try_replace(&mut io, &name, cc) {
        Ok(()) => {}
