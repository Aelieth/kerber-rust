...
echo "$DIFF" | grep -q '"case":"unknown-cname","outcome":"ok","error_code":6' || die "unknown-cname was not C_PRINCIPAL_UNKNOWN(6)"
echo "$DIFF" | grep -q '"case":"etype-nosupp","outcome":"ok","error_code":14' || die "missing ETYPE_NOSUPP(14)"
echo "$DIFF" | grep -q '"case":"wrong-realm","outcome":"ok","error_code":6' || die "wrong-realm was not MIT C_PRINCIPAL_UNKNOWN(6)"
