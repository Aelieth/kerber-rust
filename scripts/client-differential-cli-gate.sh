#!/usr/bin/env bash
# CLI error paths, GSS, and Z-cells. Attaches to the flows KEEP container.
...
if [ "$(docker inspect -f '{{.State.Running}}' "$NAME" 2>/dev/null)" != true ]; then
    die "client-differential-cli-gate needs MIT container (run client-differential-flows-gate.sh with KERBER_CLIENT_DIFF_KEEP=1 first)"
fi
docker exec "$NAME" test -s /tmp/proxy-krb5.conf \
    || die "missing proxy conf (flows half did not KEEP)"
docker exec "$NAME" test -s /tmp/cdiff/mit-plain.jsonl \
    || die "missing flow captures (flows half did not KEEP)"
...
log "client.diff.gate" "ok" ",\"cli_errors\":8,\"z71_till\":true,\"z82_rtime\":true"
echo "client-differential-cli-gate ok"
