...
need_bins krb5-kinit krb5-klist krb5-kvno
...
echo "$MIT_P"
echo "$MIT_P" | awk -F'Flags: ' '/host\/testhost.kerber.test/{p=1;next} p&&/Flags:/{print $2;exit}' | grep -q P
RUST_P="$(docker exec "$NAME" /tmp/krb5-klist -f -c /tmp/krb5cc_dr_rust)"
echo "$RUST_P"
echo "$RUST_P" | awk -F'Flags: ' '/host\/testhost.kerber.test/{p=1;next} p&&/Flags:/{print $2;exit}' | grep -q P
